...
class Pointnet2(nn.Module):
    def __init__(self, number_coarse,input_dim=0,):
        super().__init__()
...
class Model(nn.Module):
    def __init__(self, args, num_pred=6144, grid_size=4, global_feature_size=1024):
        ...
        self.backbone_modules = Pointnet2(self.number_coarse)
    ...
    def forward(self,pc_input):
        ...
        global_features,coarse_features = self.backbone_modules(pc_input)  #B 1024 1  B 1024 M

        features = torch.cat([coarse_features,global_features.expand(-1,-1,self.number_coarse)],dim=1) #B 2048 M


        coarse = self.mlp(features).transpose(1,2) # B M 3
        
        rebuild_feature = torch.cat([
            global_features.transpose(1,2).expand(-1, self.number_coarse, -1),
        ...
        return (coarse.contiguous(), pc_out.contiguous())
