...
class Model(nn.Module):
    ...
    def forward(self,pc_input,sep_pc = None,cen_gt = None,train = False):

        bs , n , _ = pc_input.shape

        if train:
            center_features = self.backbone_modules1(pc_input,sep_pc)  #B 1024
        ...
        if train:
            ...
            if sep_pc is not None:
                sep_pc = sep_pc - cen_gt.unsqueeze(-2)
        else:
            pc_input = pc_input - center.unsqueeze(-2)
            if sep_pc is not None:
                sep_pc = sep_pc - center.unsqueeze(-2)

                

        global_features = self.backbone_modules2(pc_input,sep_pc)
