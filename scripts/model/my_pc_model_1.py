...
class Pointnet2(nn.Module):
    def __init__(self, input_dim=0, size_z=128, global_feature_size=1024):
        ...
        self.second_conv = nn.Sequential(
            ...
            nn.BatchNorm1d(512),
            nn.ReLU(inplace=True),
            nn.Conv1d(512,global_feature_size,1)
        )
        ...
        self.lin3 = torch.nn.Linear(2048, 1024)
    ...
    def forward(self, pointcloud,sep_pc = None):
        bs , n , _ = pointcloud.shape

        feature = self.first_conv(pointcloud.transpose(2,1))  # B 256 n
        feature_global = torch.max(feature,dim=2,keepdim=True)[0]  # B 256 1
        feature = torch.cat([feature_global.expand(-1,-1,n), feature], dim=1)# B 512 n
        feature = self.second_conv(feature) # B 1024 n
        feature_global = torch.max(feature,dim=2,keepdim=False)[0] # B 1024
        ...
        sep_features = self.first_conv(sep_xyz.transpose(2,1))  # B 256 n
        sep_features = torch.max(sep_features,dim=2,keepdim=True)[0]  # B 256 1
        sep_features = torch.cat([sep_features.expand(-1,-1,4), sep_features], dim=1)# B 512 n
        sep_features = self.second_conv(sep_features) # B 1024 n
        sep_features = torch.max(sep_features,dim=2,keepdim=False)[0] # B 1024


        features = torch.cat((feature_global,sep_features), dim=1)# B 2048 
        out = F.relu(self.bn1(self.lin1(features)))
